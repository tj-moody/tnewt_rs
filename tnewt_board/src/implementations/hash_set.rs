...
impl Board {
    ...
    fn pawn_gen_moves(
            ...
            if target_square.is_none() {
                ...
                if rank == invalid_ranks[1] {
                    for promotion_move in &PromotionMove::from_move(mov) {
                        let mov = Move::PromotionMove(*promotion_move);
                        if !pseudo_legal {
                            self.push_if_legal(mov, moves);
                        } else {
                            moves.insert(mov);
        ...
        if file != 7 {
            ...
            if (!Piece::is_same_color(start_square, target_square) && !target_square.is_none())
                || Some(target_index) == self.state.ep_index
            {
                let basic_move = BasicMove {
                    start_index,
                    target_index,
                };
                if rank == invalid_ranks[1] {
                    for promotion_move in &PromotionMove::from_move(basic_move) {
                        let mov = Move::PromotionMove(*promotion_move);
                        if !pseudo_legal {
                            self.push_if_legal(mov, moves);
                        } else {
    ...
    fn gen_castling_moves(&mut self, moves: &mut HashSet<Move>, pseudo_legal: bool) {
        self.state
            ...
            .for_each(|&mov| {
                ...
                if let Move::BasicMove(_) = mov {
                    panic!("`CastlingState.get_moves()` returned `BasicMoves`s");
                } else if let Move::CastlingMove(castling_move) = mov {
            ...
            if let Some(piece) = square {
                ...
                match piece.kind {
                    PieceKind::Bishop | PieceKind::Rook | PieceKind::Queen | PieceKind::King => {
                        self.slider_gen_moves(&mut moves, start_index, piece, true)?
                    }
                    PieceKind::Knight => self.knight_gen_moves(&mut moves, start_index, true),
                    PieceKind::Pawn => self.pawn_gen_moves(&mut moves, start_index, true)?,
                };
            }
        }
        self.gen_castling_moves(&mut moves, true);
    ...
    fn pseudo_legal_is_legal(&mut self, mov: &Move) -> bool {
        let mut king_indices: Vec<usize>;
        if let Move::CastlingMove(cm) = mov {
...
impl Playable<HashSet<Move>> for Board {
    ...
    fn gen_legal_moves(&mut self) -> Result<HashSet<Move>, board::Error> {
        ...
        for start_index in 0..64 {
            ...
            if let Some(piece) = square {
                ...
                match piece.kind {
                    PieceKind::Bishop | PieceKind::Rook | PieceKind::Queen | PieceKind::King => {
                        self.slider_gen_moves(&mut moves, start_index, piece, false)?
                    }
                    PieceKind::Knight => self.knight_gen_moves(&mut moves, start_index, false),
                    PieceKind::Pawn => self.pawn_gen_moves(&mut moves, start_index, false)?,
                };
            }
        }
        self.gen_castling_moves(&mut moves, false);
    ...
    fn make_move(&mut self, mov: &Move) -> Result<(), board::Error> {
        ...
        let basic_move: BasicMove = match mov {
            Move::BasicMove(bm) => *bm,
            ...
            Move::PromotionMove(pm) => pm.pawn_move,
        };

        let ep_taken_index: Option<usize> = (|| {
            if let Some(index) = self.state.ep_index {
                if basic_move.target_index != index {
                    return None;
                }
                if let Some(piece) = self.squares[basic_move.start_index] {
                    if piece.kind != PieceKind::Pawn {
                        return None;
                    }
                    return Some(match color {
                        Color::White => basic_move.target_index + 8,
                        Color::Black => basic_move.target_index - 8,
                    });
                }
            }
            None
        ...
        let moving_piece = self.squares[basic_move.start_index];
        let captured_piece = self.squares[basic_move.target_index];

        if self.algorithm == board::Algorithm::Unmove {
            self.state.last_ep_taken_index = ep_taken_index;
            self.state.last_move = Some(*mov);
            self.state.last_captured_square = Some(self.squares[basic_move.target_index]);
        }
        ...
        match moving_piece {
            Some(piece) => match piece.kind {
                ...
                PieceKind::Rook => match basic_move.start_index {
                    0 | 56 => self
                        .state
                        .castling_state
                ...
                PieceKind::Pawn => {
                    self.state.halfmove_clock = 0;
                    let offset: isize = basic_move.target_index as isize - basic_move.start_index as isize;
                    self.state.ep_index = match offset {
                        -16 => Some(basic_move.start_index - 8),
                        16 => Some(basic_move.start_index + 8),
                        _ => None,
                    };
                }
    ...
    fn unmake_move(&mut self) -> Result<(), board::Error> {
        ...
        match last_move {
            Move::BasicMove(bm) => {
                ...
                let last_moved_square = self.squares[bm.target_index];
                self.squares[bm.target_index] = last_captured_square;
                self.squares[bm.start_index] = last_moved_square;
            }
            Move::CastlingMove(cm) => {
