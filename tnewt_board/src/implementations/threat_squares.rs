...
// use crate::board::*;
use crate::magic_numbers::{DIRECTION_OFFSETS, KNIGHT_XY_OFFSETS, SQUARES_TO_EDGE, THREAT_SQUARES, ALL_SQUARES};
...
impl Board {
    ...
    fn index_gen_pseudo_legal_moves(&self, indices: &[usize], ) -> Result<Vec<Move>, board::Error> {
    ...
    fn attacked(&mut self, index: usize) -> Result<bool, board::Error> {
        ...
        let moves = board.index_gen_pseudo_legal_moves(THREAT_SQUARES[index])?;
...
impl Playable<Vec<Move>> for Board {
    ...
    fn gen_legal_moves(&mut self) -> Result<Vec<Move>, board::Error> {
        let mut moves = self.index_gen_pseudo_legal_moves(ALL_SQUARES)?;
        moves.retain(|mov| {
            ...
            match self.algorithm {
                board::Algorithm::Clone => {
                    ...
                    let mut threat_indices = check_indices
                        .iter()
                        .map(|&i| THREAT_SQUARES[i])
                        .collect::<Vec<_>>()
                        .concat();
                    threat_indices.dedup();

                    opponent_responses = board.index_gen_pseudo_legal_moves(threat_indices.as_slice()).unwrap();
                }
                board::Algorithm::Unmove => {
                    ...
                    opponent_responses = self.index_gen_pseudo_legal_moves(threat_indices.as_slice()).unwrap();
